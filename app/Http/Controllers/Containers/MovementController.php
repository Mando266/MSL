...
class MovementController extends Controller
{
    public function index()
    {
        ...
        if(Auth::user()->id == 14){
            $filteredData = Movements::filter(new ContainersIndexFilter(request()))->orderBy('id')
            ->whereHas('container.containersOwner', function ($query) {
                $query->where('id', 1);
            })->orWhere(function ($query2) {
                $query2->whereHas('movementcode.containerstatus', function ($qq) {
                    $qq->where('id', 1);
                })
                ->whereHas('container.containersOwner', function ($qqq) {
                    $qqq->where('id', 6);
                });
            })->groupBy('container_id')->with('container')->get();
            $movementsBlNo = Movements::where('company_id',Auth::user()->company_id)->select('bl_no')
            ->whereHas('container.containersOwner', function ($query) {
                $query->where('id', 1);
            })->orWhere(function ($query2) {
                $query2->whereHas('movementcode.containerstatus', function ($qq) {
        ...
        if(request('movement_id') != null || request('port_location_id') != null){
            ...
            if(Auth::user()->id == 14){
                ...
                })->orderBy('movement_date','desc')->with('movementcode.containerstock','container')->get();
            }else{
            ...
            if(request('movement_id') != null){
                foreach($filteredData as $key => $move){
                    // Get All movements and sort it and get the last movement before this movement 
                    if(Auth::user()->id == 14){
                        $tempMovements = Movements::where('container_id',$move->container_id)->orderBy('movement_date','desc')
                        ->whereHas('container.containersOwner', function ($query) {
                            $query->where('id', 1);
                        })->orWhere(function ($query2) {
                            $query2->whereHas('movementcode.containerstatus', function ($qq) {
            ...
            if(request('port_location_id') != null){
                foreach($filteredData as $key =>$move){
                    // Get All movements and sort it and get the last movement before this movement 
                    if(Auth::user()->id == 14){
                        $tempMovements = Movements::where('container_id',$move->container_id)
                        ->whereHas('container.containersOwner', function ($query) {
                            $query->where('id', 1);
                        })->orWhere(function ($query2) {
                            $query2->whereHas('movementcode.containerstatus', function ($qq) {
        ...
        if(request('TillDate')){
            ...
            // Get All movements and sort it and get the last movement before this movement 
            if(Auth::user()->id == 14){
                $movements = Movements::where('container_id',$move->container_id)->orderBy('movement_date','desc')
                ->whereHas('container.containersOwner', function ($query) {
                    $query->where('id', 1);
                })->orWhere(function ($query2) {
                    $query2->whereHas('movementcode.containerstatus', function ($qq) {
                ...
                $movements = Movements::where('container_id',$move->container_id)->orderBy('movement_date','desc')->with('movementcode.containerstock')->get();
            }
            ...
            $movements = $new;
            // End Get All movements and sort it and get the last movement before this movement 
            $lastDCHF = $lastMove = $movements->where('movement_id',ContainersMovement::where('code','DCHF')->pluck('id')->first())->first();
        }else{
            $tillDate = null;
            $periods = null;
            $lastDCHF = null;
        ...
        if(count($temp) != 1){
            ...
            if(request('movement_id') == null && request('port_location_id') == null){
                ...
                $movements = $movements->orderBy('movement_date','desc')->orderBy('id','desc')->with('movementcode.containerstock')->paginate(30);
                ...
                if(request('container_id') != null){
                    ...
                    // prepare Data for export
                    $exportmovements = Movements::where('container_id',$container_id)->orderBy('movement_date','desc')->with('movementcode.containerstock')->get();
                    $exportmovements = $exportmovements->groupBy('movement_date');
                    ...
                    // prepare Data for show
                    if(Auth::user()->id == 14){
                        $movements = Movements::where('container_id',$container_id)->orderBy('movement_date','desc')
                        ->whereHas('container.containersOwner', function ($query) {
                            $query->where('id', 1);
                        })->orWhere(function ($query2) {
                            $query2->whereHas('movementcode.containerstatus', function ($qq) {
                        ...
                        $movements = Movements::where('container_id',$container_id)->orderBy('movement_date','desc')->with('movementcode.containerstock')->paginate(30);
                    }
                    $new = $movements->getCollection();
                    $new = $new->groupBy('movement_date');
    ...
    // dd($request->input());
        ...
        foreach($request->movement as $move){
            ...
            // Get All movements and sort it and get the last movement before this movement 
                $movements = Movements::where('container_id',$move['container_id'])->orderBy('movement_date','desc')->with('movementcode')->get();
                ...
                $movements = $new;
                $lastMove = $movements->where('movement_date','<=',$request->input('movement_date'))->pluck('movement_id')->first();
            // End Get All movements and sort it and get the last movement before this movement 
    ...
    public function show($id)
    {
        ...
        if(request('plNo') == null){
            ...
            $movements = Movements::filter(new ContainersIndexFilter(request()))->where('container_id',$id)->with('movementcode')->where('bl_no',request('plNo'))->orderBy('movement_date','desc')->orderBy('id','desc');
        }
    ...
    public function update(Request $request, Movements $movement)
    {
        ...
        $movements = Movements::where('container_id',$movement->container_id)->orderBy('movement_date','desc')->with('movementcode')->get();
        ...
        $movements = $new;
        $lastMove = $movements->where('movement_date','<=',$movement->movement_date)->where('id','!=',$movement->id)->pluck('movement_id')->first();
        // End Get All movements and sort it and get the last movement before this movement
