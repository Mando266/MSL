...
class BlDraftController extends Controller
{
    ...
    {
        $booking  = Booking::orderBy('id','desc')->where('booking_confirm',1)->
        where('has_bl',0)->where('company_id',Auth::user()->company_id)->with('customer')->get();
        $transhipments = Booking::orderBy('id','desc')->where('is_transhipment',1)->
        where('has_bl',0)->where('company_id',Auth::user()->company_id)->with('customer')->get();
