                ...
                <div class="widget widget-one">
                    <div class="widget-heading">
                        <nav class="breadcrumb-two" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="">Demurrage</a></li>
                                <li class="breadcrumb-item active"><a href="javascript:void(0);">Demurrage Calculation</a></li>
                            </ol>
                        </nav>
                    </div>
                        ...
                        <form action="{{route('storage.store')}}" method="POST">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-4">
                                    <label>BL NO</label>
                                    <select class="selectpicker form-control" id="blno" data-live-search="true" name="bl_no" data-size="10" title="{{trans('forms.select')}}" required>
                                        @foreach($movementsBlNo as $item)
                                            <option value="{{$item->id}}" {{$item->id == old('bl_no',isset($input) ? $input['bl_no'] : '') ? 'selected':''}}>{{$item->ref_no}} - {{$item->equipmentsType->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="Date">Container No</label>
                                    <select class="selectpicker form-control" id="port" data-live-search="true" name="container_code[]" data-size="10" title="{{trans('forms.select')}}" required multiple>
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select class="selectpicker form-control" data-live-search="true" name="service" data-size="10" id="service" title="{{trans('forms.select')}}" required>
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-3">
                                    ...
                                    <select class="selectpicker form-control" id="triff_id" data-live-search="true" name="Triff_id" data-size="10" title="{{trans('forms.select')}}" required></select>
                                </div>
                                <div class="form-group col-md-3">
                                    <label>From</label>
                                    <select class="selectpicker form-control" data-live-search="true" name="from" data-size="10" title="{{trans('forms.select')}}">
                                ...
                                <div class="form-group col-md-3">
                                    <label>To</label>
                                    <select class="selectpicker form-control" data-live-search="true" name="to" data-size="10" title="{{trans('forms.select')}}">
                                        @foreach ($movementsCode as $item)
                                    ...
                                    <tbody>
                                        @foreach($calculation['containers'] as $item)
                                            <tr>
                                                ...
                                                <td class="col-md-2" style="border-right-style: hidden;">From: {{$item['from']}} <br>To: {{$item['to']}}</td>
                                                ...
                                                <td class="col-md-2 text-center">{{$item['total']}}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                    <tfoot>
                                        <tr>
                                            ...
                                            <td class="col-md-2 text-center">{{$calculation['grandTotal']}}</td>
                                        </tr>
                                    </tfoot>
                                </table>
                            @endisset
...
@push('scripts')
    ...
    <script>
        let selectedCodes = '{{ implode(',',$input['container_code'] ??[]) }}'.split(',');
        ...
        $(document).ready(function () {
            const getTriff = () => {
                ...
                $.get(`/api/storage/triffs/${service.val()}/${company_id}`).then(function (data) {
                    ...
                    for (let i = 0; i < triffs.length; i++) {
                        list2.push(`<option value="${triffs[i].id}" ${triffs[i].id == selectedTriff ? 'selected':''}>${triffs[i].tariffTypeCode} ${triffs[i].portsCode} ${triffs[i].validfrom} ${triffs[i].validto} ${triffs[i].containersType}</option>`);
                    }
                    let triff = $('#triff_id');
            ...
            const getContainers = () => {
                ...
                axios.get(`/api/bl/is-export/${bl.val()}`).then(response => {
                ...
                $.get(`/api/storage/bl/containers/${bl.val()}/${company_id}`).then(function (data) {
                    let containers = data.containers || '';
                    let list2 = [`<option value='all' ${selectedCodes[0] == 'all' ? 'selected' : ''}>All</option>`];
                    for (let i = 0; i < containers.length; i++) {
                        list2.push(`<option value='${containers[i].id}' ${selectedCodes.includes(containers[i].id.toString()) ? 'selected' : ''}>${containers[i].code}</option>`);
                    }
                    let container = $('#port');
...
console.log(triff);
            if (cart.some(item => item.blNo === blNo && item.triffValue === triffValue)) {
                swal("Warning", "You cannot add the same data twice Please Clear Cart.", "warning");
                return;
            }
        ...
        function updateCartDisplay() {
            ...
            cart.forEach((item, index) => {
                ...
                const containers = new Set(item.calculationData.containers.map(container => `From Date: ${container.from} Till Date: ${container.to}`));
                const text = Array.from(containers).join(',');
                itemDetails.innerHTML = `<div>${text}</div>`;
        ...
        createInvoiceButton.addEventListener('click', () => {
            ...
            blNoForInvoice.value = cart[0].blNo;
