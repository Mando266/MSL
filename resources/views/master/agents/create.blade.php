        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 layout-spacing">
            <div class="widget widget-one">
                ...
                <form id="createForm" action="{{route('agents.store')}}" method="POST">
                        ...
                        <div class="form-row">
                        <div class="form-group col-md-4">
                                <label for="countryInput">Agent Type</label>
                                <select class="selectpicker form-control" id="countryInput" data-live-search="true" name="agent_type_id" data-size="10"
                                 title="{{trans('forms.select')}}">
                                    @foreach ($agent_types as $item)
                                        <option value="{{$item->id}}" {{$item->id == old('agent_type_id') ? 'selected':''}}>{{$item->name}}</option>
                                    @endforeach
                                </select>
                                @error('agent_type_id')
                                <div class="invalid-feedback">
